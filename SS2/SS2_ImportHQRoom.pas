	...
	begin
		...
		for i:=0 to relevantContainers.count-1 do begin
			...
			for j:=0 to assets.count-1 do begin
				...
				end else if(strStartsWithCI(curRes, 'Meshes\SS2C3\Interface\GNNRoomShapes')) then begin
					if(listModelsMisc.indexOf(curRes) < 0) then begin
						listModelsMisc.add(regexReplace(curRes, '^Meshes\\SS2C3\\Interface\\GNNRoomShapes\\', ''));
					end;
				end;
				updateProgress(j);
			end;
