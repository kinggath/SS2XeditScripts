...
unit userscript;
    ...
    var
        baseMatPath, baseModelsPath, baseName, descr, nameForEdid, modelPath: string;
        hasColors: boolean;
        omodSwap: IInterface;
        baseObj: TJsonObject;
    ...
    begin
        e := getExistingElementOverrideOrClosest(e, targetFile);
        // figure out:
        //  - Base material path:   find the MaterialSwaps property, that thing's path
        //  - Base models path to parent directory for flag models: ???*
        //  - Base name: FULL without the "Base Material - " prefix
        //  - Material immersive description: DESC
        //  - Supports Color Indexing? Y/N: has SS2_Tag_FactionFlag_HasBaseColorSelected?


        {
        ___
        *
            The hardest piece to look up here will be the Base Flag Models Path. For that, you’ll want to fetch each
            unique SS2_FactionFlag_FlagDown_B* static (ie. anything where the portion of the EDID between FlagDown_B and
            _BC is unique) and grab all the models from there.
        }
    ...
    var
        omodSwap: IInterface;
        baseName, baseMatPath, nameForEdid: string;
        hasColors: boolean;
        baseObj: TJsonObject;
    begin
        e := getExistingElementOverrideOrClosest(e, targetFile);
