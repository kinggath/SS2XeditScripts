...
unit PexParser;
    ...
    var
        ...
        pexStringTable, pexContainedObjects, pexExtendedObjects: TStringList;
    ...
    begin
		...
		for i:=0 to cnt-1 do begin
			...
			// sometimes we don't even have these
    ...
    var
		...
		curExtendedObject, curContainedObject: string;
    begin
        // getStringTableEntry
        ...
        if(size > 0) then begin
            ...
            curContainedObject := trim(pexGetStringTableEntry(nameIndex));
            curExtendedObject := trim(pexGetStringTableEntry(parentClassIndex));

			if(curContainedObject <> '') then begin
				pexContainedObjects.add(curContainedObject);
			end;

			if(curExtendedObject <> '') then begin
				pexExtendedObjects.add(curExtendedObject);
			end;
	...
	function _readPexByteStream(byteStream: TFileStream): boolean;
    begin
        Result := false;
        ...
        try
			...
			pexBr.free();
			pexBr := nil;
			pexCurrentStream := nil;
        end;
	end;

	function readPexScriptName(scriptName: string): boolean;
	begin
		Result := readPexResource(_scriptNameToPexPath(scriptName));
	end;

	function readPexResource(filename: string): boolean;
	var
...
//AddMessage('Parsing '+filename+' from '+foundContainer);
		// ok try loading
		byteStream := TBytesStream.Create(ResourceOpenData(foundContainer, filename));
		Result := _readPexByteStream(byteStream);
		byteStream.free();
	end;

	{
		Recursively checks if the given script is the same as or is extending the given base script
	}
	function checkScriptExtends(scriptNameToCheck, baseScriptName: string): boolean;
	var
		i, j: integer;
		curScriptName: string;
	begin
		baseScriptName := LowerCase(baseScriptName);
		// trivial case
		if(LowerCase(scriptNameToCheck) = baseScriptName) then begin
			Result := true;
			exit;
		end;
		...
		Result := false;
		if(not readPexScriptName(scriptNameToCheck)) then begin
			AddMessage('WARNING: failed to parse '+scriptNameToCheck);
			exit;
		end;

		for j:=0 to pexExtendedObjects.count-1 do begin
			curScriptName := pexExtendedObjects[j];

			if(checkScriptExtends(curScriptName, baseScriptName)) then begin
				Result := true;
				exit;
			end;
		end;
	...
	{
        Find a script, either by it's name, or by what it extends
    }
    function findScriptInElementByName(e: IInterface; scriptName: String): IInterface;
    var
    ...
    begin
        ...
        for i := 0 to ElementCount(scripts)-1 do begin
			...
			if (checkScriptExtends(curScriptName, scriptName)) then begin
				Result := curScript;
				exit;
			end;
        end;
